...
#ifndef DEBUG_iRRAM_INSIDE
#define DEBUG_iRRAM_INSIDE 0
#endif
...
REAL REAL::mp_addition (const double i) const
{
  REAL x=i;
  return mp_addition(x);
}
...
{
  #if DEBUG_iRRAM_INSIDE >= 5000
  std::cout<<"start swrite: w="<<w<<" form="<<form<<std::endl;
  #endif
  if ( !x.value ){
    REAL y(x);
    y.mp_make_mp();
  ...
  #if DEBUG_iRRAM_INSIDE >= 5000
  std::cout<<"after value decision"<<std::endl;
  #endif
  std::string result;
  ...
  char* erg;
  #if DEBUG_iRRAM_INSIDE >= 5000
  std::cout<<"before float_type"<<std::endl;
  #endif
  if (form==iRRAM_float_absolute) {
    #if DEBUG_iRRAM_INSIDE >= 5000
    std::cout<<"start iRRAM_float_absolute"<<std::endl;
    #endif
    sizetype psize; 
    int p=-10*(width-8)/3;
    ...
    #if DEBUG_iRRAM_INSIDE >= 5000
    std::cout<<"start reiterate decide"<<std::endl;
    #endif
    if (sizetype_less(psize,x.error)|| (s>p && mantissa+8 < width) ) {
      #if DEBUG_iRRAM_INSIDE >= 5000
      std::cout<<"we need iterate"<<std::endl;
      #endif
    ...
    #if DEBUG_iRRAM_INSIDE >= 5000
    std::cout<<"start s and p decision"<<std::endl;
    #endif
    if (s<=p) {
...
// REAL fmod (const REAL& x, const REAL& y) {
// 	REAL result = x / y;
// 	REAL value001;
// 	if (MP_sign(result.value)==1) value001=result-REAL(0.5);
//     else value001 = result+REAL(0.5);
// 	INTEGER n = value001.as_INTEGER();
// 	result = x - n*y;
// 	return result;
// }
