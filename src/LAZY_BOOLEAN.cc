...
LAZY_BOOLEAN operator ||
   (const bool& x, const LAZY_BOOLEAN& y)
   {
     if (x || y.value==1) return 1;
     if (x && y.value==0) return 0;
     return LAZY_BOOLEAN::BOTTOM;
   }

LAZY_BOOLEAN operator ||
   (const LAZY_BOOLEAN& x, const bool& y)
   {
     if (x.value==1 || y) return 1;
     if (x.value==0 && y) return 0;
     return LAZY_BOOLEAN::BOTTOM;
   }
